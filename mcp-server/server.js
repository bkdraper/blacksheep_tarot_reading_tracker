...
export class TarotTrackerMCPServer {
  ...
  async callTool(toolName, args, forBedrock = false) {
    console.log(`[MCP Server] callTool called with toolName: ${toolName}, args:`, JSON.stringify(args), `forBedrock: ${forBedrock}`);
    let result;
    
    try {
      switch (toolName) {
        case 'get_session_summary':
          console.log('[MCP Server] Calling getSessionSummary...');
          result = await this.getSessionSummary(args);
          break;
        
        case 'get_top_locations':
          console.log('[MCP Server] Calling getTopLocations...');
          result = await this.getTopLocations(args);
          break;
        
        case 'get_recent_sessions':
          console.log('[MCP Server] Calling getRecentSessions...');
          result = await this.getRecentSessions(args);
          break;
        
        case 'get_reading_records':
          console.log('[MCP Server] Calling getReadingRecords...');
          result = await this.getReadingRecords(args);
          break;
      ...
      console.log('[MCP Server] Tool execution completed, result:', JSON.stringify(result));
      
      // For Bedrock Agent, return the parsed JSON directly
      if (forBedrock) {
        console.log('[MCP Server] Parsing result for Bedrock...');
        const parsedResult = JSON.parse(result.content[0].text);
        console.log('[MCP Server] Parsed result for Bedrock:', JSON.stringify(parsedResult));
        return parsedResult;
      }
      
      console.log('[MCP Server] Returning MCP format result');
      return result;
    } catch (error) {
      console.error('[MCP Server] Error in callTool:', error);
  ...
  async getSessionSummary(args) {
    console.log('[MCP Server] getSessionSummary called with args:', JSON.stringify(args));
    const { user_name, start_date, end_date } = args;
    
    console.log('[MCP Server] Building Supabase query...');
    let query = supabase
    ...
    console.log('[MCP Server] Executing Supabase query...');
    const { data, error } = await query;
    
    if (error) {
      console.error('[MCP Server] Supabase error:', error);
      throw new Error(`Database error: ${error.message}`);
    }
    
    console.log(`[MCP Server] Query returned ${data?.length || 0} sessions`);
    ...
    console.log('[MCP Server] Calculated result:', JSON.stringify(result));
